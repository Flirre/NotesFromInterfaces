...
*** Advantages
 The user does not have to actively think about saving a file.
*** Disadvantages
If the auto-save doesn't work as intended and work gets lost the trust for the software takes a huge hit.

* Chapter 15
#+LATEX: \newpage

** What is modeless feedback?
Feedback that is "just there", no popups are created or acknowledgement is needed from the user.
It is preferable because it does not disturb or require special actions.
** Explain why rich modeless feedback is so [insert numerous positive adjectives] useful. Provide an example.
If an error can be handled quietly or an action can be performed without further acknowledgement from the user than that is preferable since it won't interrupt the users flow with unnecessary excise.

** When and how should audible feedback be used?
Try to use positive audible feedback and avoid negative audible feedback.
Use it to indicate that something happened right now. Acknowledge that something is happening.
Use it to indicate that something happened in the background.

** How can the conventional undo feature be made more powerful?
If you can undo changes after the document has been saved and/or closed.
If there are several (preferably many) steps that you can undo.
If you could undo several actions at once if they can be grouped logically. Example: Undo all the boldings I have done in this paragraph so that they are ordinary text.

Be clear with what is undone. Example: "Undo Bold text", "Undo resize image" etc.

* Chapter 16
#+LATEX: \newpage
** Give examples of pedagogic, immediate and invisible commands respectively.
*** Pedagogic
"In your face", labeled (in) menus. Slow but good for beginner. "Save as.." found by clicking "File" in the menus.
*** Immediate
Available and visible. Example Toolbars. Can act as reminders. Bolding text etc. Easy access, no label.
*** Invisible
Hidden, gestures, keyboard shortcuts. Ctrl+C = Copy, Kinect gestures for different actions. C-x-s = save.

** Providing help or hints to users as they try to learn a new interface is important. List five ways in which this can be done in graphical interfaces.
Tooltips. Labeling shortcuts/accelerators in menus. Guided tours. Wizards. Templates. Overlays.
** Give five design choices to consider when it comes to accessibility and graphical interfaces.
Try to make the interface traversable with just a keyboard for disabled users.
Try to use symbols as well as colors, to avoid alienating color blind users.
The font should be able to be customized (enlarged, bolded, etc.).
Make the interface usable for people with reduced vision by making the interface readable by a synthesized voice and with alt. tags for icons and pictures etc.
Avoid blinking, flickering etc. not just because it is annoying but also because it can be hazardous for some users who suffer from seizures.

* Chapter 17
#+LATEX: \newpage
** Which visual attributes do graphic components have?
Size, color, texture, position, orientation, shape, motion/change

** Why are all-caps texts more difficult to read?
The letters are less distinct when all-caps, making it harder to distinguish what letters the user is reading.

** What are the benefits of using a grid as basis for design?
It groups items. You can use gaps to group them.

** Apart from using a grid, which principles should be applied when laying out the components of an interface to create structure and flow?

** Does a layout grid decrease or increase visual excise? Why?
It decreases it. It creates an order that the order percieves (unconscious) and that helps the user understand the structure and order of the interface. It also groups content.

** How can you establish a visual hierarchy in a graphical interface?
Show the user what is most important in the most prominent manner.
You can use position, putting things at the top or in the middle, etc.
Colour, size, etc. can also establish the visual hierarchy.
...
** In terms of “anatomy of a desktop application”, which are the main building blocks?
Primary window, secondary windows, Different panes.

** How should menus be designed to be “pedagogic vectors”? Give examples.
Try to follow standards. /File/ in the same location.
Pliancy on current objects, checkmarks to indicate if a mode is selected.

** What is the difference between a toolbar and a tool palette and when should they be used, respectively?
Toolbars use visual icons and perhaps a small text label to clarify. They are used for *performing actions on selections*.
Tool palettes use text only. These *switch between states/tools*.

** What is cascading menus? What are the advantage and disadvantage with them?
When a menu item is selected a secondary 'side-menu' pops out with more menu items.
More 'levels' make them *harder to navigate* and hides options.
It enables the designer to *fit more tools* in the menus.

** What is a tool tips?
An information box that shows up when the mouse is hovered over an element. This explains what the element/button does.

** When is an action committed; on mouse down or mouse up? Why?
*Mouse up*. To *confirm/commit the action*.

** What are the advantages with an object-verb oriented selection sequence?
+ You can *perform several actions in a row* on an object.
+ After selection you can show the commands that are possible to apply to that object.

** What types of selection are there and when and why do they apply? (read more?)
+ Hierarchy / category, example: choosing text by heading etc. Choosing layers in Photoshop.
+ Arbitrary by additive selection (with modifier key and mouse click) or by dragging an marquee.

** What characterizes a well-designed drag and drop sequence?
 Show what is being moved with a selection box. While moving the item show possible drop candidates. When dropping the item keep the selection of the item while displaying it in its new location.
* Chapter 19
** What are the advantages with touch interaction?
+ It allows for *direct manipulation*, changing things by touching and moving them with your fingers.
+ Allows for *multi-touch*. Example: zooming on maps.
** What are the disadvantages with touch interaction?
+ Finger pointing is not precise.
+ Occlusion, you cover up parts of the screen while interacting with it.
+ One-hand interaction is difficult, especially for users with small hands. Hard to reach all of the screen with one hand.
+ No hover/mouse over hinting. So no dynamic visual hinting pliancy and tool-tips are eliminated.
+ No right-click capabilities.

** How can some of the disadvantages with touch interaction be minimised?
+ Have big target size, can even extend out and be bigger than the icon it is attached to.
+ Hint pliancy, 3D buttons with shadows etc.
+ Plan for potential occlusion and show relevant information where the hand is not in the way.

** List the gestures that are used for touch interaction interfaces
+ Tap to - select/activate/toggle
+ Drag/swipe to - scroll/move/change/open
+ Pinch to - zoom (spatial or semantic)
+ pinch to - rotate (map) or turn (knob)
+ tap and hold - uncommon
+ multifinger swipe - avoid

** How can the search interface on mobile (and other) applications be made more efficient and powerful?
+ Content carousels.
+ Hinting about scrolling possibilities.
+ 'swim lanes' - Combination of carousels and grids. Like Netflix.
+ auto-completion based on writing patterns or previous searches.

+ pop-up control panels / drawers
+ tool-bar similar to desktop. Hide tool-configuration inside the tool. Activated after tapping the tool. Make it easy to tap and use and avoid actions such as drag-and-drop. No hierarchies.
+ mimic the interface of the physical hardware that you are 'emulating'.

* Chapter 20

** Cooper claims that desktop applications are only really needed for a specific kind of tools; which ones?
Sophisticated authoring tools such as:
+ graphic design
+ 3d modelling
+ video editing

** What is a hamburger menu?
It is a way of hiding the primary navigation by only being able to access it by pressing a button.

** According to Cooper, is scrolling something to avoid or to embrace?
Embrace it, but enhance it.
Enhance it by having both light and dark areas that you scroll through. Or clear separators.
Keep the primary navigation.

** How can we create a responsive web design?
Have resizing options up to a certain breakpoint. When that breakpoint is reached you change to a layout made for a different screen. Example: From desktop layout to mobile layout, adjusted from 16:9 to 9:16.
The mobile versions should account for screen size, touch input, sensors and challenging light situations.

* Chapter 21
** What is the difference between imperative controls, selection controls, and entry controls?

** Which imperative controls are there, and when and why should they be used, respectively?

** Which selection controls are there, and when and why should they be used, respectively?

** Which entry controls are there, and when and why should they be used, respectively?

** What are the advantages and disadvantages with bounded and unbounded entry controls?

** What’s wrong with error messages?

** How can confirmation dialogs be avoided?
